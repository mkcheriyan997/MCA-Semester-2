...
public class GameController {
        ...
        try {
            ...
            if ("BUDGET_QUIZ".equals(type)) {
                ...
                if (score == 5) {
                    ...
                    response.put("message", "Perfect Score! You earned $500 and +10 Knowledge.");
                } else if (score >= 3) {
                    stats.setMoney(stats.getMoney() + 200);
                    stats.setKnowledge(stats.getKnowledge() + 5);
                    response.put("message", "Good Job! You earned $200 and +5 Knowledge.");
                } else {
                    response.put("message", "Better luck next time!");
                }
                advanceTime(profile);
            } else if ("INVESTMENT".equals(type)) {
                String risk = (String) request.getOrDefault("risk", "LOW");
                double amount = Double.parseDouble(request.getOrDefault("amount", "0").toString());

                if (stats.getMoney() < amount || amount <= 0) {
                    return ResponseEntity.badRequest().body("Invalid investment amount.");
                }
                ...
                if ("HIGH".equals(risk)) {
                    if (rnd.nextBoolean()) {
                        outcome = amount * 3.0; // 3x return
                        response.put("message", "Incredible success! High-risk investment paid off!");
                    } else {
                        outcome = 0; // Lost everything
                        response.put("message", "Market crashed! You lost your entire investment.");
                    }
                } else { // LOW risk
                    if (rnd.nextDouble() < 0.90) { // 90% chance of small gain
                        outcome = amount * 1.20;
                        response.put("message", "Steady growth. Safe investment yielded a 20% return.");
                    } else { // 10% chance of small loss
                        outcome = amount * 0.80;
                        response.put("message", "Minor dip. Safe investment lost 20%.");
                    }
                }

                stats.setMoney(stats.getMoney() + outcome);
                advanceTime(profile);
            } else {
    ...
    private void checkAndRecordMilestones(PlayerProfile profile) {
        ...
        if (stats.getMoney() >= 100000 && stats.getMoney() < 100300) {
            recordTimelineEvent(profile, "💰 Reached $100,000 in savings! Financial security achieved.", "MONEY", profile.getAge(), profile.getCurrentWeek());
        }
        if (stats.getMoney() >= 1000000 && stats.getMoney() < 1000500) {
            recordTimelineEvent(profile, "🏆 MILLIONAIRE! Your wealth crossed $1,000,000!", "MONEY", profile.getAge(), profile.getCurrentWeek());
        }
        // Knowledge milestones
        if (stats.getKnowledge() >= 100 && stats.getKnowledge() < 115) {
            recordTimelineEvent(profile, "📚 Became a domain expert with 100+ knowledge points.", "CAREER", profile.getAge(), profile.getCurrentWeek());
        }
        // Relationship milestone
        if (stats.getRelationships() >= 90 && stats.getRelationships() < 95) {
            recordTimelineEvent(profile, "❤️ Your social network is thriving. Deep bonds formed.", "RELATIONSHIP", profile.getAge(), profile.getCurrentWeek());
        }
        // High stress warning
