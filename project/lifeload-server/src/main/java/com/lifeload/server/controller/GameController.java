    ...
    public ResponseEntity<?> playMiniGame(@RequestBody Map<String, Object> request) {
        try {
            ...
            if ("BUDGET_QUIZ".equals(type)) {
                ...
                response.put("message", "Deep Work Complete! Score: " + score + ". Earned $" + (int)credits + " and +" + motivationBonus + " Motivation.");
                advanceTime(profile);
                ...
                response.put("message", "Memory Matrix Complete! Reached Level " + score + ". Earned $" + (int)credits + " and +" + knowledgeBonus + " Knowledge.");
                advanceTime(profile);
            } else if ("TYPING_HUSTLE".equals(type)) {
                int score = (Integer) request.getOrDefault("score", 0);
                
                // New logic: $4 per point (since each word is now 5 points), +1 Reputation per 25 points
                ...
                response.put("message", "Typing Hustle Complete! " + score + " words typed. Earned $" + (int)credits + " and +" + repBonus + " Reputation.");
                advanceTime(profile);
            } else {
